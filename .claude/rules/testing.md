...
- Real-Sankhya smoke tests (spec §15) never run in GitHub Actions: Sankhya credentials exist only in the worker container (SNK-1). They run manually or on a schedule from the staging environment against the SNK-3 environment, and only after V-11 closes.
